...
export interface MessagesCallState extends CallState {
...
export const useMessagesWebRTC = (
  selectedChatId: string,
  otherParticipant: any,
  token: string,
  userId: string
) => {
  const [callState, setCallState] = useState<MessagesCallState>({
    isInCall: false,
    callType: null,
    participants: [],
    localStream: null,
  ...
  useEffect(() => {
    if (selectedChatId && token && userId) {
      ...
      realTimeServiceRef.current.connect(selectedChatId).then(() => {
        console.log('Real-time messaging service connected');
      }).catch(console.error);
  ...
  const startAudioCall = useCallback(async () => {
    try {
      ...
      setCallState(prev => ({
        ...
        isInCall: true,
        callType: 'audio',
        participants: [otherParticipant?.userId || otherParticipant?.id],
        localStream: stream,
        connectionStatus: 'connecting',
        isAudioEnabled: true,
      ...
      // Log call start if we have a conversation
      if (selectedChatId && selectedChatId !== '1' && selectedChatId !== '2' && selectedChatId !== '3' && selectedChatId !== '4') {
        await logCallStart(selectedChatId, {
          callId,
          callType: 'audio',
          participants: [userId, otherParticipant?.userId || otherParticipant?.id],
  ...
  }, [selectedChatId, otherParticipant, userId, token]);
  ...
  const startVideoCall = useCallback(async () => {
    try {
      ...
      // Initiate call
      webSocketService.initiateCall(otherParticipant?.userId || otherParticipant?.id, callId, 'video');
      setCallStatus('ringing');
  ...
  }, [incomingCall, selectedChatId, userId, token]);
  ...
  const endCall = useCallback(async () => {
    ...
    try {
      ...
      // End the call
      webSocketService.endCall(otherParticipant?.userId || otherParticipant?.id, currentCallIdRef.current);
  ...
  }, [selectedChatId, callState.callType, userId, otherParticipant, token]);
  ...
  const toggleScreenShare = useCallback(async () => {
    try {
      if (callState.isScreenSharing) {
        ...
        // Log screen share end if we have a conversation
        if (selectedChatId && selectedChatId !== '1' && selectedChatId !== '2' && selectedChatId !== '3' && selectedChatId !== '4') {
          await logScreenShareEnd(selectedChatId, token);
        }
        ...
        // Log screen share start if we have a conversation
        if (selectedChatId && selectedChatId !== '1' && selectedChatId !== '2' && selectedChatId !== '3' && selectedChatId !== '4') {
          await logScreenShareStart(selectedChatId, token);
        }
  ...
  }, [callState.isScreenSharing, selectedChatId, token]);
